(function(){
  ...
  // Read QID directly from urlParams or sessionStorage (so it survives F5 but doesn't auto-fill from pre-questionnaire on same device)
  var questionarioId = urlParams.get('questionario_id') || sessionStorage.getItem('pos_active_questionario_id');
  var sessionId = null;
  ...
  var currentSection = 0; // 0 = sec-researcher-select or sec-offer
  ...
  function showSection(idStr) {
    ...
    else currentSection = 0; // sec-researcher-select or sec-offer

    updateProgress();
    window.scrollTo(0,0);
  }

  function loadPosWaitingList() {
    var listContainer = document.getElementById('pos-waiting-list');
    if(!listContainer) return;
    
    listContainer.innerHTML = '<div style="font-size:.85rem; color:var(--ink-soft);">Buscando participantes aguardando...</div>';
    fetch(NAO_API + '/questionario/lista')
      .then(function(r) { return r.json(); })
      .then(function(data) {
         var aguardando = data.questionarios ? data.questionarios.filter(function(q) { return q.status === 'jogo_concluido'; }) : [];
         listContainer.innerHTML = '';
         if(aguardando.length === 0){
            listContainer.innerHTML = '<div style="font-size:.85rem; color:var(--ink-faint); padding:12px; background:var(--surface-soft); border-radius:8px; text-align:center;">Nenhum participante aguardando o p\u00f3s-question\u00e1rio.</div>';
         } else {
         ...
         btnReload.className = 'btn btn-outline';
         btnReload.style.marginTop = '10px';
         btnReload.style.justifyContent = 'center';
         btnReload.innerHTML = 'Atualizar lista';
         btnReload.onclick = loadPosWaitingList;
         listContainer.appendChild(btnReload);
      })
      .catch(function(){});
  }

  // Auto-load participant if selected, otherwise show researcher screen
  function loadParticipant() {
    if(!questionarioId) {
      showSection('sec-researcher-select');
      loadPosWaitingList();
      return;
    }
